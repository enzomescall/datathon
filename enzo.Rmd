...
#missing: china??, annual income quintiles, main occupation
```

```{r model}
model <- lm(avg ~ country + gender + marital_status + urban_rural + age + education + household_members + household_generations + subjective_status + religion, data = final_data) %>%
  tidy() %>%
  kable(digits = 3)
model
```